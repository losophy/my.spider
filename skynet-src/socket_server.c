...
struct socket {
	...
	uint8_t type;						// epoll事件触发时，会根据type来选择处理事件的逻辑（读、写、监听等）
...
socket_server_create() {
	...
	// 将 pipe 的读端放入 epoll 中监听，注意 pipe 消息是没有 socket* 变量的，为NULL
...
socket_server_poll(struct socket_server *ss, struct socket_message * result, int * more) {
	for (;;) {
		...
		case SOCKET_TYPE_LISTEN: {											// 由于使用了异步tcp连接，连接成功后，服务器listen_fd可读。
