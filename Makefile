...
# jemalloc 
# jemalloc是一種內存分配算法，比c語言的malloc效率高
...
#foreach的語法是這樣$(foreach <var>,<list>,<text>)， 它會從list中挨個取出var，作用於text
all : \
...
#核心目標skynet。依賴於一堆的src文檔。下面的$@，$^兩個叫做自動化變量。$@上面講了，$^是所有的依賴集。
define CSERVICE_TEMP
  $$(CSERVICE_PATH)/$(1).so : service-src/service_$(1).c | $$(CSERVICE_PATH)
	$$(CC) $$(CFLAGS) $$(SHARED) $$< -o $$@ -Iskynet-src
endef

#define是定義命令包，endif結尾，CSERVICE_TEMP是命令包的名字。$(1)的意思是第一個參數，這裏指的是？可能是調用命令的時候傳進來的參數。
#另外$$的用法是，$$代表真實的$，而不是調用變量。
#用到了幾個函數，eval，call和addprefix。call能夠創建參數
#addprefix增加前綴。eval是將後面的參數作為makefile的一部分解析執行。define，$$，eval經常在一起使用。
...
#cleanall裏面有個wildcard函數。在變量的值裏面使用的通配符
cleanall: clean
